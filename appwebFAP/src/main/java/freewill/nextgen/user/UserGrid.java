...
public class UserGrid extends Grid {
    ...
    private StringToEnumConverter roleConverter = new StringToEnumConverter() {
        @Override
        public String convertToPresentation(@SuppressWarnings("rawtypes") Enum point,
            ...
            if (point == UserRoleEnum.COORD) {
                text = "Federación";
            } 
            else if (point == UserRoleEnum.USER) {
            	text = "Juez";
            }
    ...
    public UserGrid() {
        ...
        getColumn("role").setConverter(roleConverter).setRenderer(new HtmlRenderer());
