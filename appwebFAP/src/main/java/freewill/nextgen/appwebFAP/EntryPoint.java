...
import freewill.nextgen.resultados.AnonymousLogin;
...
public class EntryPoint extends UI {
    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        if(vaadinRequest.getParameter("resultados")!=null 
           || vaadinRequest.getPathInfo().contains("resultados")){
            System.out.println("Invoking Resultados Page...");
            addStyleName(ValoTheme.UI_WITH_MENU);
            // wrapper a ResultadosCrudView para mostrar marco y configurar usuario Anónimo
            AnonymousLogin results = new AnonymousLogin(accessControl);
            setContent(results);
            return;
        }
    ...
    protected void drawMainView() {
        ...
        heartbeat = new FeederThread(
        		...
        		"AppwebFAP");
        // TODO mientras debug heartbeat.start();
    }
	...
	/*Component header(){
        ...
        Image image = new Image(null, new ThemeResource("img/LogoFAP90x90.png"));
	...
	public void userLogout() {
        ...
        RtdbDataService.get().pushEvent(new EventEntity(
				new Date(), 
				String.format(AlarmDic.ALM0023.toString(), username, "AppwebFAP"),
				username,
    ...
    public int getConfigInt(ConfigItemEnum key){
		try {
			ConfigEntity rec = (ConfigEntity) BltClient.get().executeCommand("/getconfig/"+key.name(), 
					ConfigEntity.class, 
					EntryPoint.get().getAccessControl().getTokenKey());
			int value = Integer.parseInt(rec.getValue());
			return value;
		} catch (Exception e) {
			e.printStackTrace();
    ...
    public long getConfigLong(ConfigItemEnum key){
		try {
			ConfigEntity rec = (ConfigEntity) BltClient.get().executeCommand("/getconfig/"+key.name(), 
					ConfigEntity.class, 
					EntryPoint.get().getAccessControl().getTokenKey());
    ...
    public String getConfigString(ConfigItemEnum key){
		try {
			ConfigEntity rec = (ConfigEntity) BltClient.get().executeCommand("/getconfig/"+key.name(), 
					ConfigEntity.class, 
					EntryPoint.get().getAccessControl().getTokenKey());
    ...
    public boolean getConfigBoolean(ConfigItemEnum key){
		try {
			ConfigEntity rec = (ConfigEntity) BltClient.get().executeCommand("/getconfig/"+key.name(), 
					ConfigEntity.class, 
					EntryPoint.get().getAccessControl().getTokenKey());
			boolean value = Boolean.parseBoolean(rec.getValue());
			return value;
		} catch (Exception e) {
			e.printStackTrace();
        	//throw new IllegalArgumentException("Fail to retrieve Record from Config Database");
    ...
    @WebServlet(urlPatterns = "/*", name = "AppwebFAP", asyncSupported = true)
