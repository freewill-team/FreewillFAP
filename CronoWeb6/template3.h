...
<script>
  ...
  function setThreshold(crono, val){
    var bio = document.getElementById(crono);
    bio.innerHTML = val;
    console.log('Into setThreshold()');
  }
  ...
  function showGateStatus(element, state){
    ...
    if(state.includes('OK'))
      ...
      setThreshold('crono', state);
      slider.value = parseInt(state);
    }
  }
...
<body onload="javascript:start();">
  ...
  <div class="crono_wrapper">
    <input type="range" min="1" max="1023" id="myRange" class="slider">
    <h1 id='crono'>0000</h1>
  </div>
