...
public class ReportRFI extends Report {
	
	@SuppressWarnings("deprecation")
	public ReportRFI(String name, String report, ProjectEntity prj, 
			CompanyEntity cpy,
		...
		try{
        	...
        	List<RequirementMapping> reqs = getmapsbyproject(
        			prj.getID(), cpy.getID(), 
        			reqsrepo, mapsrepo, userrepo, featrepo, productrepo);
			
			for(RequirementMapping req:reqs){
				doc.AddTitle(req.getCustomid()+" "+req.getDescription(), 1);
				doc.AddParagraph(req.getText());
				
				if(req.getDoc()==null || req.getDoc()<1L) continue;
				
				// Gets the list of Features
				List<FeatureEntity> recs = new ArrayList<FeatureEntity>();
				FeatureEntity feat = featrepo.findById(req.getDoc());
				recs.add(feat);
				getfeatbyparent(req.getDoc(), recs, featrepo);
				System.out.println("Docs to print =  "+recs.size());
				int delta = 0;
				if(recs.size()>0){
					int level = recs.get(0).getLevel().ordinal();
					delta = 2 - level;
				}
				...
				for(FeatureEntity rec : recs){
					...
					if(level.ordinal()<=StyleEnum.H8.ordinal()){
						doc.AddTitle(title, level.ordinal()+delta);
						figtitle = title;
					}
	...
	private void getfeatbyparent(Long parent, List<FeatureEntity> output,
			FeatureRepository repo) throws Exception {
		List<FeatureEntity> recs = repo.findByParentOrderByIdAsc(parent);
		for(FeatureEntity rec : recs){
			output.add(rec);
			getfeatbyparent(rec.getID(), output, repo);
		}
	}
