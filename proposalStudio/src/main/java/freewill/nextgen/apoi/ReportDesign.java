...
public class ReportDesign extends Report {
	...
	public ReportDesign(String name, String report, Long project, Set<Long> products, ProgressBar pb){
		try{
			// Set ProgressBar 
        	pb.setValue(0f);
			...
			// Stores the list of requirements
			List<RequirementMapping> reqsmapped = (List<RequirementMapping>) 
				...
				for(FeatureEntity rec : recs){
			    	...
			    	for(MappingEntity map : maps){
			    		...
			    		if(req!=null){
				    		...
				    		// Update reqsmapped with feature info
				    		//updateReqsMapped(reqsmapped, req.getID(), rec, prd.getName());
			    		}
			...
			doc.AddTitle(Messages.get().getKey("TOC"), 1);
    		doc.AddParagraph("");
    		...
    		table[0][3]=Messages.get().getKey("mappedto");
    		table[0][4]=Messages.get().getKey("product");
    		int j=1;
	...
	/*private void updateReqsMapped(List<RequirementMapping> reqsmapped, 
			Long reqId, FeatureEntity feat, String product) {
		// Updates reqsmapped using req and map info
		for(RequirementMapping rec:reqsmapped){
			if(rec.getId()==reqId){
	        	...
	        	if(feat.getTitle().equals("") && feat.getParent()!=null && feat.getParent()>0L){
	        		FeatureEntity fp = featureLogic.findRecord(feat.getParent());
